...
const VerifyPage = () => {
  ...
  const handleInputChange = (index: number, value: string): void => {
    if (value.length > 1) {
      //cannot enter multiple value in a first otp inputs in UI
      return;
    }
    const newOtp = [...otp];
    newOtp[index] = value;
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>): void => {
    e.preventDefault(); // protect submission upon pasting immediately!
      ...
      <div className="max-w-md w-full">
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-8">
          <div className="text-center mb-8 relative">
            ...
            <p className="text-gray-400">We have send a 6 digit code to </p>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-4 text-center">
                Enter your 6 digits otp code here
              </label>
              <div className="flex justify-center in-checked:space-x-3">
                {
                  // parenthesis after callback arrow so that we donot need to return ;
                  otp.map((digit, index) => (
                    <input
                      key={index}
          ...
          <div className="mt-6 text-center">
            <p className="text-gary-400 text-sm mb-4">
